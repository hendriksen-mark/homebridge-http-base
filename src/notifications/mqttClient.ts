...
export class MQTTClient extends EventEmitter {
  ...
  _addSubscription(subscription: MQTTSubscription) {
    ...
    if (Object.prototype.hasOwnProperty.call(this.subscriptions, subscription.topic)) {
...
this.client!.end();
    } // mqtt library would try to reconnect every second

    this.log.error('MQTT error occurred: ' + error.message);
    this.emit('error', error);
  }

  _message(topic: string, payload: Buffer) {
